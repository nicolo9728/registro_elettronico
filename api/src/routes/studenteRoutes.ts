...
studenteRoutes.get("/voti", controlloLoggato, controlloStudente, async (req ,res)=>{
    ...
    try{
        const ris = await new Pool(dbImpostazioni).query("SELECT valutazione, data, nomeMateria, descrizione, nome FROM VOTI natural join Docenti where idStudente=$1", [matricola])
